...
static func input_enabled() -> bool:
	return GameInput.is_enabled(GameInput.InputMode.PLAYER)
...
static func get_direction() -> float:
	if is_locked() or not input_enabled():
		return 0.0
...
static func get_looking_direction(default: Vector2 = Vector2.RIGHT, enable_8_way_snapping: bool = true) -> Vector2:
	if not input_enabled():
		return default
	var vector: Vector2 = Input.get_vector("move_left", "move_right", "move_up", "move_down")
	if enable_8_way_snapping:
		return snap_to_8_way(vector, default)
	return vector
...
static func snap_to_8_way(vector: Vector2, default: Vector2 = Vector2.RIGHT) -> Vector2:
