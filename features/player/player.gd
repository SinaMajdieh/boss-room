...
## animation, and combat-related systems.
...
@export var animation: PlayerAnimationTree
...
## Animation player for attack-related animation.
...
func _process(_delta: float) -> void:
	...
	if Input.is_action_just_pressed("apply_knock_back"):
		movement.apply_knock_back(-2.0 * velocity)
...
func hurt(
	amount: Variant = 1,
	knock_back: Vector2 = Vector2.ZERO
) -> void:
	if not can_take_damage():
		return

	var hurt_state : PlayerState = state_machine.get_node_state("hurt")
	hurt_state.set_attributes(amount, knock_back)
