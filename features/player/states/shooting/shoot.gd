...
@export var info: ShootingResource = ShootingResource.new()
...
func play_animation() -> void:
	if animations.is_animation_playing(info.shoot_turn_animation):
		return
	if player.is_on_floor():
		if is_zero_approx(player.velocity.x):
			play_shoot_animation(info.shoot_animation[aim_direction], (1 / info.shoot_cool_down_time))
		else:
			play_shoot_animation(info.shoot_run_animation)
	else:
		play_shoot_animation(info.air_shoot_animation)
...
func _on_turn_around() -> void:
	if not player.is_on_floor():
		return
	animations.play_animation(info.shoot_turn_animation)
