...
func play_animation() -> void:
	...
	if player.is_on_floor():
		if is_zero_approx(player.velocity.x):
			animations.play_animation(info.shoot_animation[aim_direction])
...
func next_stage() -> void:
	if charge_state_index < player_bullet_scene.size() - 1:
		charge_state_index += 1
		var new_bullet: ChargeBullet = player_bullet_scene[charge_state_index].instantiate() as ChargeBullet
...
func release_bullet() -> void:
	...
	player.shoot_cool_down.start(info.shoot_cool_down_time)
